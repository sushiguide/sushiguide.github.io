...
const I18N = {
  uk: {
    ...
    hero: 'Чесний гід по суші міста. Оцінка за 5 категоріями, ціна та умови доставки.',
    ...
    statusActive: 'Працюють', statusAll: 'Усі заклади', statusTemp: 'Тимчасово закриті', statusClosed: 'Закриті',
    statusTempShort: 'Тимчасово закритий', statusClosedShort: 'Закритий',
    statusClosedNote: 'Заклад закритий. Оцінка збережена з останньої дегустації.',
    statusTempNote: 'Тимчасово не працює. Оцінка з останньої дегустації.',
    emptyList: 'Нічого не знайдено за цими фільтрами',
    ...
    scoreLabel: 'Оцінка',
    price: 'Ціна', priceDisc: 'Зі знижкою',
  ...
  en: {
    ...
    hero: 'An honest sushi guide for the city. Score across 5 categories, price and delivery terms.',
    ...
    statusActive: 'Open', statusAll: 'All places', statusTemp: 'Temporarily closed', statusClosed: 'Closed',
    statusTempShort: 'Temporarily closed', statusClosedShort: 'Closed',
    statusClosedNote: 'This place is closed. Score kept from the last tasting.',
    statusTempNote: 'Temporarily closed. Score from the last tasting.',
    emptyList: 'Nothing matches these filters',
    ...
    scoreLabel: 'Score',
    price: 'Price', priceDisc: 'With discount',
  ...
  try {
    ...
    document.getElementById('app').innerHTML = `
      <div class="text-center py-20 text-red-500">
        Не вдалося завантажити контент.<br>
        <span class="text-sm text-zinc-400">${e.message}</span>
      </div>`;
  }
}
...
function placeStatus(p) {
  ...
  if (s === 'closed' || s === 'temporary') return s;
  return 'open';
}

function statusBadge(p) {
  const s = placeStatus(p);
  if (s === 'closed') {
    return `<span class="text-xs px-2 py-0.5 rounded-full bg-zinc-800 text-white">${t('statusClosedShort')}</span>`;
  }
  if (s === 'temporary') {
    return `<span class="text-xs px-2 py-0.5 rounded-full bg-amber-200 text-amber-900">${t('statusTempShort')}</span>`;
  }
  return '';
}
...
function cardStatusClass(p) {
  const s = placeStatus(p);
  if (s === 'closed') return 'opacity-60 grayscale border-zinc-300';
  if (s === 'temporary') return 'border-amber-300 ring-1 ring-amber-200';
  return '';
}
...
function statusBanner(p) {
  const s = placeStatus(p);
  if (s === 'closed') {
    return `<div class="mb-5 rounded-lg bg-zinc-900 text-white text-sm px-3 py-2.5">${t('statusClosedNote')}</div>`;
  }
  if (s === 'temporary') {
    return `<div class="mb-5 rounded-lg bg-amber-50 text-amber-950 text-sm px-3 py-2.5 border border-amber-200">${t('statusTempNote')}</div>`;
  }
  return '';
}
...
function deliveryBlock(p) {
  ...
  if ((fee === '—' || fee === '–') && (min === '—' || min === '–' || min == null) && (free === '—' || free === '–' || free == null)) {
    return `<div class="bg-zinc-50 rounded-lg p-3 text-sm mb-5">
      <div class="font-medium mb-1">${t('delivery')}</div>
      <div class="text-zinc-600">${t('noDelivery')}</div>
      ${note ? `<div class="text-zinc-500 text-xs mt-1">${note}</div>` : ''}
    </div>`;
  }
  ...
  return `<div class="mb-5">
    <h4 class="font-bold text-sm mb-2">${t('delivery')}</h4>
    <div class="grid grid-cols-3 gap-2 text-sm mb-2">
      <div class="bg-zinc-50 rounded-lg p-2.5">
        <div class="text-zinc-500 text-xs">${t('delMin')}</div>
        <div class="font-semibold">${fmt(min)}</div>
      </div>
      <div class="bg-zinc-50 rounded-lg p-2.5">
        <div class="text-zinc-500 text-xs">${t('delFee')}</div>
        <div class="font-semibold">${fmt(fee)}</div>
      </div>
      <div class="bg-zinc-50 rounded-lg p-2.5">
        <div class="text-zinc-500 text-xs">${t('delFree')}</div>
        <div class="font-semibold">${fmt(free)}</div>
      </div>
    </div>
    ${note ? `<div class="text-xs text-zinc-500">${note}</div>` : ''}
  </div>`;
}
...
function render() {
  ...
  const st = stEl ? stEl.value : 'active';

  let list = places.filter(p => {
    if (rec && p.recommend !== rec) return false;
    if (cat && p.category !== cat) return false;
    const ps = placeStatus(p);
    if (st === 'active' && ps !== 'open') return false;
    if (st === 'closed' && ps !== 'closed') return false;
    if (st === 'temporary' && ps !== 'temporary') return false;
    return true;
  });
  ...
  if (!list.length) {
    document.getElementById('grid').innerHTML = `<div class="col-span-full text-center py-16 text-zinc-400">${t('emptyList')}</div>`;
    return;
  }
  ...
  document.getElementById('grid').innerHTML = list.map(p => {
    ...
    return `
    <div class="card bg-white rounded-xl border p-5 cursor-pointer ${cardStatusClass(p)}" onclick="openModal(${p.id})">
  ...
  document.getElementById('mBody').innerHTML = `
    ...
    <div class="flex items-center gap-3 mb-5">
      ...
      <div>
        ...
        <div class="mt-1 flex flex-wrap gap-1.5">${statusBadge(p)}${recBadge(p.recommend)}${incompleteBadge(p)}</div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-2 gap-3 text-sm mb-5">
      ...
      <div class="bg-zinc-50 rounded-lg p-3">
        <div class="text-zinc-500 text-xs">${t('priceDisc')}</div>
        <div class="font-semibold">${p.priceDiscount ? p.priceDiscount + ' грн' : '—'}</div>
      </div>
    </div>
...
function catRow(label, val, max) {
  ...
  return `<div class="flex items-center gap-3">
    <div class="w-44 text-zinc-600 shrink-0">${label}</div>
    <div class="flex-1">${bar(val, max)}</div>
    <div class="w-14 text-right font-semibold">${v}<span class="text-zinc-400 font-normal text-xs">/${max}</span></div>
  </div>`;
}
...
function bindFilters() {
  ['filterRec', 'filterCat', 'sortBy', 'filterStatus'].forEach(id => {
